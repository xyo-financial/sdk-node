...
export const BASE_PATH =
  'https://api.xyo.financial'.replace(/\/+$/, '')
...
export interface ConfigurationParameters {
  ...
  accessToken?:
    ...
    | ((
        name?: string,
        scopes?: string[],
      ) => string | Promise<string>) // parameter for oauth2 security
...
export class BaseAPI {
  ...
  withMiddleware<T extends BaseAPI>(
    this: T,
  ...
  private fetchApi = async (
    ...
    try {
      response = await (
        this.configuration.fetchApi || fetch
      )(fetchParams.url, fetchParams.init)
    } catch (e) {
      for (const middleware of this.middleware) {
...
export function querystring(
  ...
  return Object.keys(params)
    .map((key) =>
      querystringSingleKey(key, params[key], prefix),
    )
...
function querystringSingleKey(
  ...
  const fullKey =
    keyPrefix + (keyPrefix.length ? `[${key}]` : key)
...
export function canConsumeForm(
  consumes: Consume[],
): boolean {
  for (const consume of consumes) {
    if (
      consume.contentType?.startsWith(
        'multipart/form-data',
      ) == true
    ) {
      return true
